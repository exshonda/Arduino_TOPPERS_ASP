 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: exception.c 50 2012-09-07 03:47:18Z ertl-hiro $
 */
 ...
 *  トレースログマクロのデフォルト定義
 */
#ifndef LOG_DEF_EXC_ENTER
#define LOG_DEF_EXC_ENTER(excno, pk_dexc)
#endif /* LOG_DEF_EXC_ENTER */

#ifndef LOG_DEF_EXC_LEAVE
#define LOG_DEF_EXC_LEAVE(ercd)
#endif /* LOG_DEF_EXC_LEAVE */
...
initialize_exception(void)
{
	...
	for (i = 0; i < TNUM_EXCNO; i++) {
		excinib_table[i].excatr = TA_NOEXS;
	}
}
 ...
 *  CPU例外ハンドラの定義
 */
#ifdef TOPPERS_def_exc

ER
def_exc(EXCNO excno, const T_DEXC *pk_dexc)
{
	EXCINIB	*p_excinib;
	ER		ercd;

	LOG_DEF_EXC_ENTER(excno, pk_dexc);
	CHECK_INIRTN();
	CHECK_PAR(VALID_EXCNO_DEFEXC(excno));
	CHECK_RSATR(pk_dexc->excatr, TARGET_EXCATR);
	CHECK_NOSPT(pk_dexc->exchdr != NULL);
	CHECK_ALIGN_FUNC(pk_dexc->exchdr);
	CHECK_NONNULL_FUNC(pk_dexc->exchdr);
	p_excinib = get_excinib(excno);

	if (p_excinib->excatr != TA_NOEXS) {
		ercd = E_OBJ;
	}
	else if ((ercd = target_check_dexc(excno, pk_dexc)) == E_OK) {
		p_excinib->excatr = pk_dexc->excatr;
		p_excinib->exchdr = pk_dexc->exchdr;
	}
