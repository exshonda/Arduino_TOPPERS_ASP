 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: dataqueue.c 71 2014-04-16 23:08:13Z ertl-hiro $
 */
 ...
 *  トレースログマクロのデフォルト定義
 */
#ifndef LOG_CRE_DTQ_ENTER
#define LOG_CRE_DTQ_ENTER(dtqid, pk_cdtq)
#endif /* LOG_CRE_DTQ_ENTER */

#ifndef LOG_CRE_DTQ_LEAVE
#define LOG_CRE_DTQ_LEAVE(ercd)
#endif /* LOG_CRE_DTQ_LEAVE */
 ...
 *  データキューの生成
 */
#ifdef TOPPERS_cre_dtq

ER
cre_dtq(ID dtqid, const T_CDTQ *pk_cdtq)
{
	DTQCB		*p_dtqcb;
	DTQINIB		*p_dtqinib;
	ATR			dtqatr;
	...
	LOG_CRE_DTQ_ENTER(dtqid, pk_cdtq);
	CHECK_INIRTN();
	CHECK_DTQID(dtqid);
  ...
  error_exit:
	LOG_CRE_DTQ_LEAVE(ercd);
	return(ercd);
}

#endif /* TOPPERS_cre_dtq */
...
snd_dtq(ID dtqid, intptr_t data)
{
	...
	CHECK_DTQID(dtqid);
	p_dtqcb = get_dtqcb(dtqid);
	CHECK_NOEXS(p_dtqcb->p_dtqinib->dtqatr);
