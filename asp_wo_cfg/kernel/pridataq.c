 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: pridataq.c 70 2014-04-16 23:04:43Z ertl-hiro $
 */
 ...
 *  トレースログマクロのデフォルト定義
 */
#ifndef LOG_CRE_PDQ_ENTER
#define LOG_CRE_PDQ_ENTER(pdqid, pk_cpdq)
#endif /* LOG_CRE_PDQ_ENTER */

#ifndef LOG_CRE_PDQ_LEAVE
#define LOG_CRE_PDQ_LEAVE(ercd)
#endif /* LOG_CRE_PDQ_LEAVE */
 ...
 *  優先度データキューの生成
 */
#ifdef TOPPERS_cre_pdq

ER
cre_pdq(ID pdqid, const T_CPDQ *pk_cpdq)
{
	PDQCB	*p_pdqcb;
	PDQINIB	*p_pdqinib;
	ATR		pdqatr;
	...
	LOG_CRE_PDQ_ENTER(pdqid, pk_cpdq);
	CHECK_INIRTN();
	CHECK_PDQID(pdqid);
  ...
  error_exit:
	LOG_CRE_PDQ_LEAVE(ercd);
	return(ercd);
}

#endif /* TOPPERS_cre_pdq */
...
snd_pdq(ID pdqid, intptr_t data, PRI datapri)
{
	...
	CHECK_PDQID(pdqid);
	p_pdqcb = get_pdqcb(pdqid);
	CHECK_NOEXS(p_pdqcb->p_pdqinib->pdqatr);
