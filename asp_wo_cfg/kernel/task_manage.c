 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: task_manage.c 73 2014-04-17 21:28:56Z ertl-hiro $
 */
 ...
 *  トレースログマクロのデフォルト定義
 */
#ifndef LOG_CRE_TSK_ENTER
#define LOG_CRE_TSK_ENTER(tskid, pk_ctsk)
#endif /* LOG_CRE_TSK_ENTER */

#ifndef LOG_CRE_TSK_LEAVE
#define LOG_CRE_TSK_LEAVE(ercd)
#endif /* LOG_CRE_TSK_LEAVE */
...
cre_tsk(ID tskid, const T_CTSK *pk_ctsk)
{
	...
	else if ((ercd = target_check_ctsk(tskid, pk_ctsk)) == E_OK) {
		if (stk == NULL) {
...
act_tsk(ID tskid)
{
	...
	p_tcb = get_tcb_self(tskid);
	CHECK_NOEXS(p_tcb->p_tinib->tskatr);
