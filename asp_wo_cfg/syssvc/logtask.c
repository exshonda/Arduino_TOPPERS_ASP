 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: logtask.c 49 2012-09-06 04:41:53Z ertl-hiro $
 */
 ...
 */
void
logtask_initialize(intptr_t exinf)
{
	T_CTSK	ctsk;
	ER		ercd;

	/*
	 *  システムログタスクの初期化処理（logtask.cfg相当の処理）
	 *
	 *  CRE_TSK(LOGTASK, { TA_ACT, LOGTASK_PORTID, logtask_main,
	 *					LOGTASK_PRIORITY, LOGTASK_STACK_SIZE, NULL });
	 */
	ctsk.tskatr = TA_ACT;
	ctsk.exinf = LOGTASK_PORTID;
	ctsk.task = logtask_main;
	ctsk.itskpri = LOGTASK_PRIORITY;
	ctsk.stksz = LOGTASK_STACK_SIZE;
	ctsk.stk = NULL;
	ercd = cre_tsk(LOGTASK, &ctsk);
	assert(ercd == E_OK);
}
