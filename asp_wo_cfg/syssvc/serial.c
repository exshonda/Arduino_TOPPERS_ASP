 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: serial.c 62 2013-10-18 15:38:33Z ertl-hiro $
 */
...
serial_initialize(intptr_t exinf)
{
	...
	/*
	 *  シリアルI/Oデバイス（SIO）ドライバの初期化
	 */
	sio_initialize(0);

	/*
	 *  シリアルインタフェースドライバの初期化処理（serial.cfg相当の処理）
	 *
	...
	csem.isemcnt = 0;
	csem.maxsem = 1;
	ercd = cre_sem(SERIAL_RCV_SEM1, &csem);
	assert(ercd == E_OK);

	csem.sematr = TA_TPRI;
	csem.isemcnt = 1;
	csem.maxsem = 1;
	ercd = cre_sem(SERIAL_SND_SEM1, &csem);
	assert(ercd == E_OK);
	...
	csem.isemcnt = 0;
	csem.maxsem = 1;
	ercd = cre_sem(SERIAL_RCV_SEM2, &csem);
	assert(ercd == E_OK);

	csem.sematr = TA_TPRI;
	csem.isemcnt = 1;
	csem.maxsem = 1;
	ercd = acre_sem(SERIAL_SND_SEM2, &csem);
	assert(ercd == E_OK);
#endif /* TNUM_PORT >= 2 */
	...
	csem.isemcnt = 0;
	csem.maxsem = 1;
	ercd = cre_sem(SERIAL_RCV_SEM3, &csem);
	assert(ercd == E_OK);

	csem.sematr = TA_TPRI;
	csem.isemcnt = 1;
	csem.maxsem = 1;
	ercd = acre_sem(SERIAL_SND_SEM3, &csem);
	assert(ercd == E_OK);
#endif /* TNUM_PORT >= 3 */
	...
	csem.isemcnt = 0;
	csem.maxsem = 1;
	ercd = cre_sem(SERIAL_RCV_SEM4, &csem);
	assert(ercd == E_OK);

	csem.sematr = TA_TPRI;
	csem.isemcnt = 1;
	csem.maxsem = 1;
	ercd = acre_sem(SERIAL_SND_SEM4, &csem);
	assert(ercd == E_OK);
#endif /* TNUM_PORT >= 4 */

	/*
	 *  シリアルポート管理ブロックの初期化（元のserial_initializeの処理）
	 *
