...
const SIOPINIB siopinib_table[TNUM_SIOP] = {
	{SERCOM4, 23u, 22u, 16u, PORTB, PIO_SERCOM_ALT, GCLK_CLKCTRL_ID_SERCOM4_CORE_Val, 115200, PM_APBCMASK_SERCOM4}
};
...
sercom_uart_devinit(SIOPCB *p_siopcb){
	...
	/* Init Clock */
	GCLK->CLKCTRL.reg = p_siopcb->p_siopinib->gcm
	...
	/* Set Pad */
	p_sercom->USART.CTRLA.reg |= SERCOM_USART_CTRLA_TXPO(0x00) | SERCOM_USART_CTRLA_RXPO(0x01);
...
sio_initialize(intptr_t exinf)
{
	...
	ercd = cfg_int(INTNO_SIO, &cint);
	assert(ercd == E_OK);

	cisr.isratr = TA_NULL;
	cisr.exinf = 1;
	cisr.intno = INTNO_SIO;
	cisr.isr = sio_isr;
	cisr.isrpri = 1;
	ercd = att_isr(&cisr);
	assert(ercd == E_OK);
