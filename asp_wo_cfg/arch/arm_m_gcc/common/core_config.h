 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: core_config.h 2859 2018-07-14 02:43:41Z ertl-honda $
 */
...
#include "interrupt.h"
#else /* ARDUINO_PASS */
#include <../asp_wo_cfg/arch/arm_m_gcc/common/core_insn.h>
#include "../asp_wo_cfg/include/queue.h"
...
{
	uint8_t iipm;
	INTINIB	*p_intinib = get_intinib(run_intno);
...
Inline ER
target_check_aisr(const T_CISR *pk_cisr)
{
	return(E_OK);
}

Inline ER
target_check_cint(INTNO intno, const T_CINT *pk_cint)
{
	return E_OK;
}
...
Inline ER
target_check_dexc(EXCNO excno, const T_DEXC *pk_dexc)
{
	return E_OK;
}
