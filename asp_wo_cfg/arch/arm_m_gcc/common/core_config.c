 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: core_config.c 2859 2018-07-14 02:43:41Z ertl-honda $
 */
...
#else
uint32_t ief[INTREG_NUM];			/* IRQの割込み要求禁止フラグの状態 */
uint8_t  ief_systick;	/* SysTickの割込み要求禁止フラグの状態 */
uint8_t  iipm;			/* 現在の割込み優先度マスクの値 */
...
target_prepare(void)
{
	...
	/* 割込みハンドラの登録 */
	for (i = TMIN_INHNO; i <= TNUM_INHNO; i++) {
		p_inhinib = get_inhinib(i);
		if (p_inhinib->inhatr != TA_NOEXS) {
			exc_tbl[INDEX_INHNO(i) + 1] = (FP)p_inhinib->inthdr;
		}
		else {
			if (i < TMIN_INTNO) {
				exc_tbl[INDEX_INHNO(i) + 1] = (FP)default_exc_handler;
			}
			else {
				exc_tbl[INDEX_INHNO(i) + 1] = (FP)default_int_handler;
			}
		}
	}

	/* 割込み要求ラインの設定 */
	for (i = TMIN_INTNO; i <= TNUM_INTNO; i++) {
		p_intinib = get_intinib(i);
		if (p_intinib->intatr != TA_NOEXS) {
			x_config_int(i, p_intinib->intatr, p_intinib->intpri);
		}
	}

	/* CPU例外の登録 */
	for (i = TMIN_EXCNO; i <= TNUM_EXCNO; i++) {
		p_excinib = get_excinib(i);
		if (p_excinib->excatr != TA_NOEXS) {
			exc_tbl[INDEX_EXCNO(i) + 1] = (FP)p_excinib->exchdr;
		}
		else {
