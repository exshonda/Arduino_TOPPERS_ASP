 ...
 */
Inline void
target_timer_register_int(void)
{
	T_CINT	cint;
	T_DINH	dinh;
	ER		ercd;
	...
	/*
	 *  タイマドライバの初期化処理（target_timer.cfg相当の処理）
	 *
	 *  CFG_INT(INTNO_TIMER, { TA_ENAINT | INTATR_TIMER, INTPRI_TIMER });
	 *  DEF_INH(INHNO_TIMER, { TA_NULL, target_timer_handler });
	 */
	cint.intatr = TA_ENAINT | INTATR_TIMER;
	cint.intpri = INTPRI_TIMER;
	ercd = cfg_int(INTNO_TIMER, &cint);
	assert(ercd == E_OK);

	dinh.inhatr = TA_NULL;
	dinh.inthdr = target_timer_handler;
	ercd = def_inh(INHNO_TIMER, &dinh);
	assert(ercd == E_OK);
}
